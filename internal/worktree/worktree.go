...
func (m *Manager) Create(slug string) (*Entry, error) {
	...
	if err != nil {
		// Fallback: if it failed because it exists, maybe isWorktree missed it
...
// List returns all manifest entries sorted by slug.
func (m *Manager) List() ([]Entry, error) {
	...
	for _, slug := range slugs {
		entries = append(entries, manifest[slug])
	}
	return entries, nil
}

// DestroyAll removes all tracked worktrees, returning the slugs removed.
// Collects errors but continues destroying remaining worktrees.
func (m *Manager) DestroyAll() ([]string, error) {
	manifest, err := m.loadManifest()
	if err != nil {
		return nil, err
	}
	...
	if len(errs) > 0 {
		return removed, fmt.Errorf("errors destroying worktrees:\n  %s", strings.Join(errs, "\n  "))
	}
	return removed, nil
}

// Exists returns true if the given slug is tracked in the manifest.
...
func isWorktree(repoRoot, path string) bool {
	...
	for _, line := range lines {
		if strings.HasPrefix(line, "worktree ") {
			gitPath := strings.TrimSpace(strings.TrimPrefix(line, "worktree "))
			realGitPath, err := filepath.EvalSymlinks(gitPath)
			if err == nil {
				gitPath = realGitPath
			}
			if filepath.Clean(gitPath) == absPath {
				return true
			}
		}
	}
