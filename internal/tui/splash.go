...
// Frames for the animated emoji splash.
...
func (m splashModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case spinner.TickMsg:
		// Advance emoji frame on every spinner tick.
		m.frame++
		var cmd tea.Cmd
...
func (m splashModel) View() string {
	...
	emoji := splashFrames[m.frame%len(splashFrames)]

	titleStyle := lipgloss.NewStyle().Bold(true).Foreground(ColorPrimary).MarginLeft(1)
	title := titleStyle.Render("MOCHI")

	header := emoji + title
	subtitle := SubtitleStyle.Render("Multi-Task AI Coding Orchestrator")
